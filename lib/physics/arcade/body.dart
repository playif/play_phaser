...
class Body {
   ...
   */
  static render  (context, Body body, [String color='rgba(0,255,0,0.4)', bool filled=true]) {
...
//    color = color || 'rgba(0,255,0,0.4)';
   ...
   */
  static renderBodyInfo  (Debug debug, Body body) {
    debug.line(['x: ' + body.x.toStringAsFixed(2), 'y: ' + body.y.toStringAsFixed(2), 'width: ' + body.width.toString(), 'height: ' + body.height.toString()]);
    debug.line(['velocity x: ' + body.velocity.x.toStringAsFixed(2), 'y: ' + body.velocity.y.toStringAsFixed(2), 'deltaX: ' + body._dx.toStringAsFixed(2), 'deltaY: ' + body._dy.toStringAsFixed(2)]);
    debug.line(['acceleration x: ' + body.acceleration.x.toStringAsFixed(2), 'y: ' + body.acceleration.y.toStringAsFixed(2), 'speed: ' + body.speed.toStringAsFixed(2), 'angle: ' + body.angle.toStringAsFixed(2)]);
    debug.line(['gravity x: ' + body.gravity.x.toString(), 'y: ' + body.gravity.y.toString(), 'bounce x: ' + body.bounce.x.toStringAsFixed(2), 'y: ' + body.bounce.y.toStringAsFixed(2)]);
    debug.line(['touching left: ' + body.touching.left.toString(), 'right: ' + body.touching.right.toString(), 'up: ' + body.touching.up.toString(), 'down: ' + body.touching.down.toString()]);
    debug.line(['blocked left: ' + body.blocked.left.toString(), 'right: ' + body.blocked.right.toString(), 'up: ' + body.blocked.up.toString(), 'down: ' + body.blocked.down.toString()]);
  }
  ...
  Body(this.sprite) {
     ...
     * @readonly
     */
    this.position = new Point(sprite.x, sprite.y);

    /**
     * @property {Phaser.Point} prev - The previous position of the physics body.
     * @readonly
     */
    this.prev = new Point(this.position.x, this.position.y);
     ...
     */
    this.center = new Point(sprite.x + this.halfWidth, sprite.y + this.halfHeight);
     ...
     * @readonly
     */
    this.newVelocity = new Point(0, 0);
     ...
     */
    this.wasTouching = new Collision()
      ..none = true
      ..up = false
      .. down = false
      .. left = false
  ...
  setSize (num width, num height,[num offsetX, num offsetY]) {
    ...
    if (offsetX == null) { offsetX = this.offset.x; }
    if (offsetY == null) { offsetY = this.offset.y; }
